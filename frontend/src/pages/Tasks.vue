<template>
  <LayoutHeader>
    ...
    <template #right-header>
      ...
      <Button
        variant="outline"
        :label="__('Update Overdue')"
        iconLeft="clock"
        @click="updateOverdueTasks"
        class="mr-2"
      />
      <Button
        variant="solid"
...
import { formatDate, timeAgo, timeToDue } from '@/utils'
...
function parseRows(rows, columns = []) {
  ...
  return rows.map((task) => {
    ...
    // Add virtual time_to_due field
    _rows['time_to_due'] = task.due_date
...
async function updateOverdueTasks() {
  try {
    const result = await call('crm.fcrm.doctype.crm_task.crm_task.check_and_update_overdue')
    if (result) {
