<template>
  ...
  >
    ...
    >
      ...
      <div v-else-if="column.key === 'time_to_due'">
        <div v-if="item" class="flex items-center gap-2 truncate text-base">
          <div><CalendarIcon /></div>
          <div class="truncate" :class="getTimeToDueInfo(item).color">
            {{ getTimeToDueInfo(item).text }}
          </div>
        </div>
...
import { formatDate, timeToDue } from '@/utils'
import {
  Avatar,
  ListView,
  ListHeader,
...
function getTimeToDueInfo(dueDate) {
  return timeToDue(dueDate)
}
