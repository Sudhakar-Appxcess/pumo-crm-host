...
def get_data(
	...
	if view_type != "kanban":
		if columns or rows:
			...
			print(f"[DEBUG] Custom view path - doctype={doctype}, rows={rows}")
		...
		if not custom_view and frappe.db.exists("CRM View Settings", default_view_filters):
			...
			print(f"[DEBUG] Using default_list_data path - doctype={doctype}")
			rows = default_rows
		...
		# Use get_all for CRM Lead due to permissions issue with get_list
		if doctype == "CRM Lead":
			print(f"[DEBUG] Calling frappe.get_all for CRM Lead with fields={rows[:3]}... filters={filters}")
			data = (
				frappe.get_all(
					doctype,
					fields=rows,
			...
			print(f"[DEBUG] frappe.get_all returned {len(data)} items")
		else:
			data = (
				frappe.get_list(
					doctype,
	...
	# Add virtual time_to_due field for CRM Task
	if doctype == "CRM Task":
		std_fields.append({"label": "Time to Due", "fieldtype": "Datetime", "fieldname": "time_to_due"})
	...
	return {
		...
		"total_count": (
			frappe.get_all(doctype, filters=filters, fields="count(*) as total_count", limit=1)[0].total_count
			if doctype == "CRM Lead"
			else frappe.get_list(doctype, filters=filters, fields="count(*) as total_count")[0].total_count
		),
		"row_count": len(data),
...
@frappe.whitelist()
def remove_assignments_with_reason(doctype, name, assignees, reason=None, ignore_permissions=False):
	"""Remove assignments with reason"""
	assignees = frappe.parse_json(assignees)
	...
	current_user_name = frappe.get_cached_value("User", current_user, "full_name")
	
	# Process each assignment removal
	for assign_to in assignees:
		set_status(
			doctype,
			name,
		...
		create_reassignment_activity_log(
			doctype, name, assign_to, "removed", 
			current_user, current_user_name, reason
		)
...
@frappe.whitelist()
def add_assignments_with_reason(doctype, name, assignees, reason=None, ignore_permissions=False):
	"""Add assignments with reason"""
	assignees = frappe.parse_json(assignees)
	...
	for assign_to in assignees:
		# Use the standard Frappe assignment
		...
		# Create activity log for reassignment
		create_reassignment_activity_log(
			doctype, name, assign_to, "assigned", 
			current_user, current_user_name, reason
		)
...
def create_reassignment_activity_log(doctype, docname, assignee, action, current_user, current_user_name, reason):
	"""Create activity log for reassignment"""
	assignee_name = frappe.get_cached_value("User", assignee, "full_name")
	
	# Create activity log entry with better formatting
	content = f"Lead Owner {action.title()} {assignee_name}"
	
	# Add reason if provided
	if reason:
		content += f"<br><strong>Description notes:</strong> {reason}"
	
	activity_log = frappe.get_doc({
		"doctype": "Activity Log",
		"reference_doctype": doctype,
		"reference_name": docname,
		"subject": f"Assignment {action}",
		"content": content,
		"user": current_user,
...
def send_reassignment_notification(doctype, docname, assignee, action, current_user, current_user_name, reason):
	"""Send notification for reassignment with reason"""
	...
	if action == "assigned":
		message = f"{current_user_name} assigned {doctype} {docname} to you"
		notification_text = f"""
			<div class="mb-2 leading-5 text-ink-gray-5">
				<span class="font-medium text-ink-gray-9">{current_user_name}</span>
				<span>assigned a {doctype} {docname} to you</span>
			</div>
		"""
	else:
		message = f"Your assignment on {doctype} {docname} has been removed by {current_user_name}"
		notification_text = f"""
			<div class="mb-2 leading-5 text-ink-gray-5">
				<span>Your assignment on {doctype} {docname} has been removed by</span>
				<span class="font-medium text-ink-gray-9">{current_user_name}</span>
			</div>
		"""
	...
	# Get redirect info
	redirect_to_doctype, redirect_to_name = get_redirect_to_doc_for_reassignment(doctype, docname)
...
def get_redirect_to_doc_for_reassignment(doctype, docname):
	"""Get redirect information for reassignment notifications"""
	if doctype == "CRM Lead":
		return "CRM Lead", docname
	elif doctype == "CRM Deal":
		return "CRM Deal", docname
	elif doctype == "CRM Task":
		return "CRM Task", docname
	else:
