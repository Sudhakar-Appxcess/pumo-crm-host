...
def get_deal_activities(name):
	...
	# Add activity log entries for task and note modifications
	activity_logs = frappe.db.get_all(
		"Activity Log",
		filters={
			"reference_doctype": "CRM Deal",
			"reference_name": name,
			"subject": ["like", "%Modified task%"]
		},
	...
	assignment_logs = frappe.db.get_all(
		"Activity Log",
		filters={
			"reference_doctype": "CRM Deal",
			"reference_name": name,
			"subject": ["like", "%Assignment%"]
		},
		fields=["name", "subject", "content", "creation", "user"],
		order_by="creation desc"
	)
	...
	# Add activity log entries for note modifications
	note_activity_logs = frappe.db.get_all(
		"Activity Log",
		filters={
			"reference_doctype": "CRM Deal",
			"reference_name": name,
			"subject": ["like", "%Modified note%"]
		},
		fields=["name", "subject", "content", "creation", "user"],
		order_by="creation desc"
	)
	
	for log in note_activity_logs:
		activity = {
			"name": log.name,
			"activity_type": "note_modified",
			"creation": log.creation,
			"owner": log.user,
			"subject": log.subject,
			"content": log.content,
...
def track_task_changes(doc, method):
	"""Track changes to CRM Task and create activity log entries"""
	if not doc.reference_docname or not doc.reference_doctype:
		return
	
	# Get the document before save to compare changes
	...
	changed_fields = []
	if doc_before.title != doc.title:
		changed_fields.append(f"title from '{doc_before.title}' to '{doc.title}'")
	if doc_before.status != doc.status:
		changed_fields.append(f"status from '{doc_before.status}' to '{doc.status}'")
	if doc_before.priority != doc.priority:
		changed_fields.append(f"priority from '{doc_before.priority}' to '{doc.priority}'")
	if doc_before.assigned_to != doc.assigned_to:
		old_user = frappe.get_value("User", doc_before.assigned_to, "full_name") if doc_before.assigned_to else "None"
		new_user = frappe.get_value("User", doc.assigned_to, "full_name") if doc.assigned_to else "None"
		changed_fields.append(f"assigned_to from '{old_user}' to '{new_user}'")
	if doc_before.due_date != doc.due_date:
		old_due = frappe.format(doc_before.due_date, {"fieldtype": "Datetime"}) if doc_before.due_date else "None"
		new_due = frappe.format(doc.due_date, {"fieldtype": "Datetime"}) if doc.due_date else "None"
		changed_fields.append(f"due_date from '{old_due}' to '{new_due}'")
	
	if changed_fields:
		# Create activity log entry
		activity_log = frappe.get_doc({
			"doctype": "Activity Log",
			"subject": f"Modified task {doc.title}",
			"content": f"Changed: {', '.join(changed_fields)}",
...
def track_note_changes(doc, method):
	"""Track changes to FCRM Note and create activity log entries"""
	if not doc.reference_docname or not doc.reference_doctype:
		return
	...
	if doc_before.title != doc.title:
		changed_fields.append(f"title from '{doc_before.title}' to '{doc.title}'")
	if doc_before.content != doc.content:
	...
	if changed_fields:
		...
		activity_log = frappe.get_doc({
			"doctype": "Activity Log",
			"subject": f"Modified note {doc.title}",
			"content": f"Changed: {', '.join(changed_fields)}",
			"reference_doctype": doc.reference_doctype,
			"reference_name": doc.reference_docname,
			"user": frappe.session.user,
